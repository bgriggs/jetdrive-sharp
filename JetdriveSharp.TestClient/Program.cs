...
class Program
{
    ...
    static async Task Main()
    {
        ...
        using ((provider = new JetdriveProvider(true, netPort, $"JETDRIVE Demo Provider", new HighAccuracyTimer())))
        {
            ...
            while (!exit)
            {
                ...
                //Transmit channel values however often you want - I wouldn't recommend going slower than 10hz though as slower values will appear kinda jerky on gauges and might make any real time code perform poorly if it expects frequent updates.
    ...
    private static void StartPostChannelsThread(JetdriveProvider _provider)
    {
        ...
        ThreadPool.QueueUserWorkItem((userState) =>
        {
            ...
            {
                JDChannelInfo info1 = new("Test RPM 1", JDUnit.EngineSpeed);
                JDChannelInfo info2 = new("Test Power", JDUnit.Power);
                ...
                ushort id1 = provider.RegisterChannel(info1);
                ushort id2 = provider.RegisterChannel(info2);
