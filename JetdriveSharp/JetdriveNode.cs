...
public abstract class JetdriveNode : IDisposable
{
    ...
    public const ushort ALL_HOSTS = 0xFFFF;
    ...
    /// </summary>
    private readonly Dictionary<ushort, byte> sequenceNumbers = [];

    /// <summary>
    /// Timestamp offsets
    /// </summary>
    private readonly Dictionary<ushort, OffsetInfo> offsets = [];
    ...
    public virtual void EraseHost(ushort hostId)
    {
    ...
    public void NegotiateHostId()
    {
        ...
        lock (sequenceNumbers)
        {
            do
            {
                hostId = (ushort)random.Next(0xFFFF);
    ...
    public void SendPing(ushort destination, byte[]? userData)
    {
        int userDataLen = 0;
        if (userData != null)
        {
        ...
        KLHDVMessage msg = new(MessageKey.Ping, this.HostId, destination, payload);
    ...
    private void SendPong(KLHDVMessage pingMsg)
    {
        ...
        byte[] payload = new byte[1 + sizeof(uint) + sizeof(uint) + echoDataLen];

        payload[0] = JETDRIVE_VERSION;

        //Copy source timestamp
        Buffer.BlockCopy(pingMsg.Value ?? [], 1, payload, 1, sizeof(uint));
        ...
        KLHDVMessage pong = new(MessageKey.Pong, this.HostId, pingMsg.Host, payload);
    ...
    private void HandlePong(KLHDVMessage pongMsg)
    {
        ...
        else
        {
            lock (offsets)
            {
                uint host_ts = BitConverter.ToUInt32(pongMsg.Value, 1);
                uint remote_ts = BitConverter.ToUInt32(pongMsg.Value, 5);

                uint currentTime = Timer.ElapsedMs;

                uint latency = (currentTime - host_ts) / 2;
    ...
    protected DateTime? CalcTimestamp(ushort host, uint ts)
    {
